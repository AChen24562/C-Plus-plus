...
private:
  ...
  float paygrade;
  int hours;
  float weekly(float p, int h); // Private function wapped
...
// Externally Defined
float Work::weekly(float p, int h){return p * h;}// Private
float Work::getWeekPay(){return weekly(paygrade, hours);} // Public calling Private
...
void printArr2(Work array[], const int size){
  for(int i = 0; i < size; i++){
    cout << "Name: " << array[i].getName() << endl;
    cout <<"Pay: "<<array[i].getPay() << endl;
    cout <<"Hours: " << array[i].getHours()<<endl;
